...
ConVar cvar_invalid_min;
ConVar cvar_invalid_max;
ConVar cvar_invalid_post;
ConVar cvar_invalid_delay;
ConVar cvar_mem;
ConVar cvar_catmem;
ConVar cvar_vote_type;
ConVar cvar_dontchange;
ConVar cvar_startsound;
ConVar cvar_endsound;
ConVar cvar_flags;
////----/CONVARS-----/////
...
char vote_group[MAP_LENGTH];

bool validity_enabled;
...
public void OnConfigsExecuted()
{
	...
	int mapmem = cvar_mem.IntValue;
	int catmem = cvar_catmem.IntValue;
...
public void OnClientPutInServer(int client)
{
	...
	int clientCount = GetRealClientCount();
	...
	if (validity_enabled && clientCount > map_max_players && view_as<PlayerLimit_Action>(cvar_invalid_max.IntValue) != PLAction_Nothing)
	{
...
public void OnClientDisconnect_Post(int client)
{
	...
	if (validity_enabled && clientCount < map_min_players && view_as<PlayerLimit_Action>(cvar_invalid_min.IntValue) != PLAction_Nothing)
	{
...
void SetupVoteSounds()
{
	...
	cvar_startsound.GetString(vote_start_sound, sizeof(vote_start_sound));
	cvar_endsound.GetString(vote_end_sound, sizeof(vote_end_sound));
...
void SetupMinMaxPlayers(const char[] map, const char[] group)
{
	// Set defaults in the event we error out.
	map_min_players = 0;
	map_max_players = MaxClients;

	umc_mapcycle.Rewind();	  // rewind the mapcycle handle
	int dmin, dmax;			  // variables to store default values for the category.
	...
	if (!StrEqual(group, INVALID_GROUP) && umc_mapcycle.JumpToKey(group))
	{
		// Store defaults for the category
		dmin = umc_mapcycle.GetNum(PLAYERLIMIT_KEY_GROUP_MIN, 0);
		dmax = umc_mapcycle.GetNum(PLAYERLIMIT_KEY_GROUP_MAX, MaxClients);
		...
		if (umc_mapcycle.JumpToKey(map))
		{
			...
			map_min_players = umc_mapcycle.GetNum(PLAYERLIMIT_KEY_MAP_MIN, dmin);
			map_max_players = umc_mapcycle.GetNum(PLAYERLIMIT_KEY_MAP_MAX, dmax);
...
void RemovePreviousMapsFromCycle()
{
	...
	FilterMapcycleFromArrays(map_kv, vote_mem_arr, vote_catmem_arr, cvar_catmem.IntValue);
}
...
public void RunPlayerLimitCheck()
{
	// Get the number of players.
	int clientCount = GetRealClientCount();
	...
	else if (validity_enabled && clientCount < map_min_players && view_as<PlayerLimit_Action>(cvar_invalid_min.IntValue) != PLAction_Nothing)
	{
...
void ChangeToValidMap(ConVar cvar)
{
	validity_enabled = false;

	switch (view_as<PlayerLimit_Action>(cvar.IntValue))
	{
		case PLAction_Now:	  // Pick a map and change to it.
		{
			...
			{
				UMC_SetNextMap(map_kv, map, group, view_as<UMC_ChangeMapTime>(cvar_invalid_post.IntValue));
			}
		}
		case PLAction_YesNo:	// Pick a map and run yes/no vote.
		{
			...
			if (UMC_GetRandomMap(map_kv, umc_mapcycle, INVALID_GROUP, map, sizeof(map), vote_group, sizeof(vote_group), false, true))
			{
				...
				{
					// Initialize the menu.
					Menu menu = new Menu(Handle_YesNoVoteMenu, MenuAction_DisplayItem | MenuAction_Display);
					...
					// Display it.
					VoteMenuToAllWithFlags(menu, cvar_vote_time.IntValue, flags);
		...
		case PLAction_Vote:	   // Run a full mapvote.
		{
			...
			if (!IsVoteInProgress())
			{
				...
				UMC_StartVote(
					...
					view_as<UMC_ChangeMapTime>(cvar_invalid_post.IntValue),				// Success Action (when to change the map)
...
public int Handle_YesNoVoteMenu(Menu menu, MenuAction action, int param1, int param2)
{
...
public void Handle_YesNoMapVote(Menu menu, int num_votes, int num_clients, const int[][] client_info, int num_items, const int[][] item_info)
{
	...
	if (item_info[0][VOTEINFO_ITEM_INDEX] == 1 && (num_votes <= 1 || item_info[0][VOTEINFO_ITEM_VOTES] != item_info[1][VOTEINFO_ITEM_VOTES]))
	{
		PrintToChatAll(
			"[UMC] %t %t (%t)",
	...
	else	// Otherwise, just display the result.
	{
		...
		UMC_SetNextMap(map_kv, map, vote_group, view_as<UMC_ChangeMapTime>(cvar_invalid_post.IntValue));
	}
}
