...
ConVar cvar_rtv_enable;
ConVar cvar_rtv_changetime;
ConVar cvar_rtv_delay;
ConVar cvar_rtv_minplayers;
ConVar cvar_rtv_postaction;
ConVar cvar_rtv_needed;
ConVar cvar_rtv_interval;
ConVar cvar_rtv_mem;
ConVar cvar_rtv_catmem;
ConVar cvar_rtv_type;
ConVar cvar_rtv_dontchange;
ConVar cvar_rtv_startsound;
ConVar cvar_rtv_endsound;
ConVar cvar_voteflags;
ConVar cvar_enterflags;
ConVar cvar_enterbonusflags;
ConVar cvar_enterbonusamt;
////----/CONVARS-----/////
...
ArrayList rtv_clients;

// Stores whether or not players have seen the long RTV message.
bool rtv_message[MAXPLAYERS + 1];

// Keeps track of a delay before we are able to RTV.
float rtv_delaystart;

// How many people are required to trigger an RTV.
int rtv_threshold;

// Flags
bool rtv_completed;	 // Has an rtv been completed?
bool rtv_enabled;		 // Is RTV enabled right now?
bool vote_completed;	 // Has UMC completed a vote?
bool rtv_inprogress;	 // Is the rtv vote in progress?
...
public void OnConfigsExecuted()
{
	...
	int mapmem = cvar_rtv_mem.IntValue;
	int catmem = cvar_rtv_catmem.IntValue;
...
void SetupVoteSounds()
{
	...
	cvar_rtv_startsound.GetString(vote_start_sound, sizeof(vote_start_sound));
	cvar_rtv_endsound.GetString(vote_end_sound, sizeof(vote_end_sound));
...
void RemovePreviousMapsFromCycle()
{
	...
	FilterMapcycleFromArrays(map_kv, vote_mem_arr, vote_catmem_arr, cvar_rtv_catmem.IntValue);
}
...
public void Handle_RTVChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
...
public void Handle_RTVMemoryChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
...
public void Handle_ThresholdChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
...
void AttemptRTV(int client)
{
	...
	if (!rtv_enabled || !cvar_rtv_enable.BoolValue || size >= rtv_threshold || client == 0)
	{
		return;
	}

	char flags[64];
	cvar_enterflags.GetString(flags, sizeof(flags));
	...
	int minPlayers = cvar_rtv_minplayers.IntValue;
	...
	if (rtv_completed || (vote_completed && cvar_rtv_postaction.IntValue == 1))
	{
	...
	else if (FindValueInArray(rtv_clients, client) == -1)
	{
		...
		cvar_enterbonusflags.GetString(flags, sizeof(flags));

		// Calc the amount of entrance points for this user
		int amt = strlen(flags) > 0 && ClientHasAdminFlags(client, flags) ? cvar_enterbonusamt.IntValue : 1;
...
void MakeRTVTimer()
{
	...
	if (rtv_delaystart > 0)
	{
		...
		CreateTimer(1.0, Handle_RTVTimer, _, TIMER_FLAG_NO_MAPCHANGE | TIMER_REPEAT);
	}
	else
	{
		// rtv_enabled = true;
...
{
	char flags[64];
	cvar_voteflags.GetString(flags, sizeof(flags));
	int count	  = GetClientWithFlagsCount(flags);
	rtv_threshold = (count > 1) ? RoundToCeil(float(count) * cvar_rtv_needed.FloatValue) : 1;
}
...
public void StartRTV()
{
	...
	rtv_clients.Clear();
	rtv_completed  = true;
	int postAction = cvar_rtv_postaction.IntValue;
	...
	else if (!vote_completed || postAction == 2)
	{
		...
		bool result = UMC_StartVote(
			...
			view_as<UMC_VoteType>(cvar_rtv_type.IntValue),						// Vote Type (map, group, tiered)
			...
			cvar_rtv_dontchange.BoolValue,										// Don't Change option
			cvar_vote_threshold.FloatValue,										// Threshold
			view_as<UMC_ChangeMapTime>(cvar_rtv_changetime.IntValue),			// Success Action (when to change the map)
