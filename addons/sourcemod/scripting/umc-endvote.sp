...
ConVar cvar_start_frags;
ConVar cvar_start_time;
ConVar cvar_start_rounds;
ConVar cvar_vote_mem;
ConVar cvar_vote_type;
ConVar cvar_vote_startsound;
ConVar cvar_vote_endsound;
ConVar cvar_vote_catmem;
ConVar cvar_vote_roundend;
ConVar cvar_flags;
ConVar cvar_delay;
ConVar cvar_changetime;
////----/CONVARS-----/////
...
//Limit Cvars
ConVar cvar_maxrounds; //Round limit cvar
ConVar cvar_fraglimit; //Frag limit cvar
ConVar cvar_winlimit; //Win limit cvar
ConVar cvar_zpsmaxrnds; // ZPS Survival
ConVar cvar_zpomaxrnds; // ZPS Objective
...
/* Forwards */
GlobalForward time_update_forward;
GlobalForward round_update_forward;
GlobalForward win_update_forward;
GlobalForward frag_update_forward;
GlobalForward time_tick_forward;
GlobalForward round_tick_forward;
GlobalForward win_tick_forward;
GlobalForward frag_tick_forward;
...
public void OnPluginStart()
{
	...
	if (cvar_maxrounds != null || cvar_winlimit != null)
	{
	...
	// ZPS Specific
	if (cvar_zpomaxrnds != null || cvar_zpsmaxrnds != null)
	{
	...
	// Forwards
	time_update_forward	 = new GlobalForward("UMC_EndVote_OnTimeTimerUpdated", ET_Ignore, Param_Cell);
	round_update_forward = new GlobalForward("UMC_EndVote_OnRoundTimerUpdated", ET_Ignore, Param_Cell);
	win_update_forward	 = new GlobalForward("UMC_EndVote_OnWinTimerUpdated", ET_Ignore, Param_Cell, Param_Cell);
	frag_update_forward	 = new GlobalForward("UMC_EndVote_OnFragTimerUpdated", ET_Ignore, Param_Cell, Param_Cell);
	time_tick_forward	 = new GlobalForward("UMC_EndVote_OnTimeTimerTicked", ET_Ignore, Param_Cell);
	round_tick_forward	 = new GlobalForward("UMC_EndVote_OnRoundTimerTicked", ET_Ignore, Param_Cell);
	win_tick_forward	 = new GlobalForward("UMC_EndVote_OnWinTimerTicked", ET_Ignore, Param_Cell, Param_Cell);
	frag_tick_forward	 = new GlobalForward("UMC_EndVote_OnFragTimerTicked", ET_Ignore, Param_Cell, Param_Cell);
}
...
public void OnConfigsExecuted()
{
	...
	if (mapcycleLoaded && cvar_endvote.BoolValue && !timer_alive)
	{
		MakeVoteTimer();
	}
...
{
	int fraglimit = cvar_fraglimit.IntValue;
	if (vote_enabled && fraglimit > 0)
	{
		...
		int startfrags = cvar_start_frags.IntValue;
...
public void Event_RoundEndZPS(Event event, const char[] name, bool dontBroadcast)
{
	if (vote_roundend)
	{
		vote_roundend = false;
...
public void Event_RestartRound(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (cvar_maxrounds != null)
	{
		// Update our vote warnings.
		Call_StartForward(round_update_forward);
		Call_PushCell(cvar_maxrounds.IntValue - cvar_start_rounds.IntValue);
		Call_Finish();
	}
	...
	if (cvar_winlimit != null)
	{
		Call_StartForward(win_update_forward);
		Call_PushCell(cvar_winlimit.IntValue - cvar_start_rounds.IntValue);
		Call_PushCell(0);
		Call_Finish();
	}
}
...
public void Event_RestartRoundZPS(Event event, const char[] name, bool dontBroadcast)
{
	round_counter = 0;

	// Update our vote warnings.
	// ZPO (Objective) maps
	if (cvar_zpomaxrnds != null && strncmp(current_map_name, "zpo_", 4) == 0)
	{
		// Update our vote warnings.
		Call_StartForward(round_update_forward);
		Call_PushCell(cvar_zpomaxrnds.IntValue - cvar_start_rounds.IntValue);
		Call_Finish();
	}
	// ZPS (Survival) maps
	else if (cvar_zpsmaxrnds != INVALID_HANDLE && strncmp(current_map_name, "zps_", 4) == 0)
	{
		// Update our vote warnings.
		Call_StartForward(round_update_forward);
		Call_PushCell(cvar_zpsmaxrnds.IntValue - cvar_start_rounds.IntValue);
		Call_Finish();
	}
	else
	{
...
void UpdateOtherTimers()
{
	...
	if (cvar_maxrounds != null)
	{
		start = cvar_maxrounds.IntValue - cvar_start_rounds.IntValue - round_counter;
		if (start > 0)
		{
	...
	if (cvar_zpomaxrnds != null && strncmp(current_map_name, "zpo_", 4) == 0)
	{
		start = cvar_zpomaxrnds.IntValue - cvar_start_rounds.IntValue - round_counter;
		if (start > 0)
		{
			LogUMCMessage("End of map vote will appear after %i more rounds.", start);
		}
	...
	// ZPS Survival Maps
	if (cvar_zpsmaxrnds != null && strncmp(current_map_name, "zps_", 4) == 0)
	{
		start = cvar_zpomaxrnds.IntValue - cvar_start_rounds.IntValue - round_counter;
		if (start > 0)
		{
	...
	if (cvar_winlimit != null)
	{
		...
		start		= cvar_winlimit.IntValue - cvar_start_rounds.IntValue - winScore;
	...
	if (cvar_fraglimit != null)
	{
		...
		start		   = cvar_fraglimit.IntValue - cvar_start_frags.IntValue - fragCount;
		if (start > 0)
		{
...
public void Handle_MaxroundsChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
	int start = StringToInt(newValue) - cvar_start_rounds.IntValue - round_counter;
...
public void Handle_WinlimitChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
	int winScore;
	int winningTeam = GetWinningTeam(winScore);
	int start		= StringToInt(newValue) - cvar_start_rounds.IntValue - winScore;
	int old			= StringToInt(oldValue);
...
// based on frags.
public void Handle_FraglimitChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
	int newlimit = StringToInt(newValue);
	if (newlimit > 0)
	{
		LogUMCMessage("End of map vote will appear after %i frags.", StringToInt(newValue) - cvar_start_frags.IntValue);
	}
	else if (StringToInt(oldValue) > 0)
	{
...
public void Handle_VoteMemoryChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
...
public void Handle_VoteChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
...
public void Handle_TriggerChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
...
{
	int startRounds = cvar_start_rounds.IntValue;
	if (cvar_winlimit != null)
	{
		int winlimit = cvar_winlimit.IntValue;
		if (winlimit > 0)
		{
...
void CheckMaxRounds()
{
	if (cvar_maxrounds != null || cvar_zpomaxrnds != null || cvar_zpsmaxrnds != null)
	{
		int maxrounds;
		...
		if (cvar_zpomaxrnds != null && strncmp(current_map_name, "zpo_", 4) == 0)
		{
			maxrounds = cvar_zpomaxrnds.IntValue;
		}
		else if (cvar_zpsmaxrnds != null && strncmp(current_map_name, "zps_", 4) == 0)
		{
			maxrounds = cvar_zpsmaxrnds.IntValue;
		}
		else
		{
			maxrounds = cvar_maxrounds.IntValue;
		}
		...
		if (maxrounds > 0)
		{
			...
			else if (cvar_clinch != null && cvar_clinch.BoolValue)
			{
				int winnerScore;
...
void GetTopTwoTeamScores(int &first, int &second = 0)
{
...
bool SetTimerTriggerTime()
{
	...
	starttime	= cvar_start_time.FloatValue * 60;
...
void SetupMapVote()
{
	if (cvar_vote_roundend.BoolValue)
	{
		vote_roundend = true;
	}
	else
...
public void StartMapVote()
{
	...
	bool result = UMC_StartVote(
		...
		cvar_extensions.IntValue > extend_counter,							// Extend option
		...
		view_as<UMC_ChangeMapTime>(cvar_changetime.IntValue),				// Success Action (when to change the map)
